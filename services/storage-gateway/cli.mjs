...
/**
 * Iyke Storage Service CLI
 * ============================================================
 * Official Command Line Interface for the Iyke Centralized Storage Platform.
 * Supports dynamic API key generation, key revocation, and cloud asset uploads.
 * ============================================================
 */
...
function printHelp() {
  ...
  console.log(`
Usage:
  node services/storage-gateway/cli.mjs <command> [subcommand] [options]
...
Examples:
  node services/storage-gateway/cli.mjs keys create --project iyke-saas --name "SaaS Mobile App"
  node services/storage-gateway/cli.mjs keys list
  node services/storage-gateway/cli.mjs keys revoke key_7a9f1234
  node services/storage-gateway/cli.mjs upload ./design.png --project iyke-portfolio --folder brand
`);
}
...
async function createKey(flags) {
  ...
  const permissions = flags.permissions ? flags.permissions.split(",").map((p) => p.trim()) : ["read", "write", "delete"];
  const allowedFolders = flags.folders ? flags.folders.split(",").map((f) => f.trim()) : ["*"];
  ...
  try {
    ...
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log("\x1b[33m⚠️  IMPORTANT: Store this key securely now. For security reasons, it will NEVER be displayed again.\x1b[0m\n");
...
async function revokeKey(keyId) {
  if (!keyId) {
    console.error("❌ Error: Key ID is required (e.g. node cli.mjs keys revoke key_12345)");
    process.exit(1);
  }
...
async function uploadFile(filePath, flags) {
